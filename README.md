...
| `llm-wiki monitor [--port <n>]` | Live read-only board view at `http://127.0.0.1:<n>` (default 4747) — who claimed what and for how long (`zcode · 14분`), claim expiry, REVIEW questions, gates/dependency waits, the terminal pile (종결 적체 — newest done/superseded/abandoned with timestamps), and an activity stream. Click any card (columns, terminal pile, activity rows) to read the full card file — Goal/AC/Plan/Notes/Handoff/Result + metadata — in a detail panel. Polls every 2s; the page renders agent-authored text via `textContent` only. **The CLI stays the only writer** — every non-GET method gets a 405; the server keeps no state |
