...
- 워치독 자동화는 **밤샘 작업이 실제로 시작될 때만 등록한다**(2026-09-24 밤 사용자 요청으로 해제 — 예정된 밤샘 작업이 없는 동안 15분 no-op 발사는 순수 소음. 제어 파일은 `.zcode/overnight/`에 그대로, 재무장은 work-loop v12 부트스트랩이나 수동 등록). **발사 성공 자체가 쿼터 생존 신호** — 쿼터 소진 중엔 발사가 모델 호출에서 실패(비용 0), 복구 후 첫 성공 발사가 stale heartbeat(20분+)를 보고 CLAIM+10분 재확인 뒤 resume 지침대로 인계한다.
