...
// 값 형태 보존 계약(2026-09-09 리뷰 5-2): 배열은 JSON 꼴로 감싸 원소의 쉼표까지
// 왕복하고, 배열·따옴표로 보이는 문자열은 역따옴표로 감싸 재파싱에서 변형되지
// 않게 한다. 따옴표 박리는 짝이 맞을 때만(비대칭 `"a'` → `a'` 문자열 보존).
function parseFrontmatter(content) {
  ...
  for (const line of match[1].split(/\r?\n/)) {
    ...
    if (quoted) {
      // 쌍따옴표는 직렬화가 JSON.stringify로 감쌌다 — 역이스케이프까지 왕복하려면
      // JSON.parse가 정확하다(내부 따옴표 `\"` 등). 실패(비JSON 레거시)면 내부 원문.
      if (quoted[1] === '"') {
        try { meta[key] = JSON.parse(trimmed); } catch { meta[key] = quoted[2]; }
      } else {
        meta[key] = quoted[2]; // 홑따옴표 — 이스케이프 규약 없음, 내부 그대로
      }
    } else if (trimmed.startsWith('[') && trimmed.endsWith(']')) {
      // JSON 꼴(원소 따옴표)이면 원소의 쉼표·괄호까지 정확히 왕복한다.
      try {
        const arr = JSON.parse(trimmed);
        meta[key] = Array.isArray(arr) ? arr.map(String) : trimmed;
      } catch {
        // 비인용 레거시 `[a, b]` — 쉼표 분해 외에 방법이 없다(원소 쉼표는 원래 손실).
        meta[key] = trimmed.slice(1, -1).split(',').map(s => s.trim()).filter(Boolean);
      }
    } else {
      meta[key] = trimmed;
    }
...
function serializeFrontmatter(meta) {
  ...
  for (const [key, value] of Object.entries(meta)) {
    if (Array.isArray(value)) {
      ...
      // `[a, b]` 모양 제목이 재파싱에서 배열로 변형되지 않게 따옴표로 감싼다.
      lines.push(`${key}: ${s.startsWith('[') || /^["']/.test(s) ? JSON.stringify(s) : s}`);
    }
  }
  lines.push('---');
  return lines.join('\n');
...
function parseBody(content) {
  ...
  if (acMatch) {
    for (const line of acMatch[1].split(/\r?\n/)) {
      ...
      if (ac) {
        card.ac.push({ checked: ac[1] === 'x', idx: Number(ac[2]), text: ac[3] });
      } else if (line.trim() && card.ac.length) {
        // 센티넬 안의 비-AC 줄(예: AC 텍스트의 줄바꿈 연속행)은 앞 항목에 붙인다 —
        // 그냥 버리면 왕복에서 소실된다(2026-09-09 리뷰 5-2).
        card.ac[card.ac.length - 1].text += '\n' + line;
      }
    }
  }
...
function moveCardTo(cardObj, targetDir, statusValue) {
  ...
  const targetPath = path.join(targetDir, cardObj.fileName);
  // 종결 폴더의 기록은 지우지 않는다(3.3) — 같은 이름이 이미 있으면 조용히 덮어쓰는
  // 대신 거부한다(2026-09-09 리뷰 5-1-3: 재생성 카드의 두 번째 done이 아카이브 Result를 소멸시켰다).
  if (fs.existsSync(targetPath)) {
    throw new Error(`종결 폴더에 같은 이름의 카드가 이미 있다: ${targetPath} — 아카이브 덮어쓰기를 거부한다. 제목을 바꿔 재생성하라.`);
  }
  const content = fs.readFileSync(cardObj.filePath, 'utf8');
  const meta = parseFrontmatter(content);
  meta.status = statusValue;
  fs.writeFileSync(targetPath, serializeCard(meta, parseBody(content)));
...
// 모든 카드 변이 명령이 통과하는 공용 락 이름. pick만의 락이 아니라 보드 전체
// 단일 쓰기자 직렬화다 — pick이 'pick', done이 'done'으로 나뉘면 서로 동시에
// 돌아 이중 상태(활성+종결 동시 존재)를 만든다.
const BOARD_LOCK_NAME = 'write';

// mkdir은 이미 존재하면 EEXIST로 실패하는 원자적 연산이라 락으로 쓴다.
// 락 폴더 안 lock.json에 {pid, at}을 남겨, LOCK_STALE_MS가 지나면 강탈 허용.
// lock.json 없는 고아 폴더(mkdir과 marker 사이에 죽은 프로세스의 흔적)는
// 폴더 mtime으로 판정해 강탈한다 — 없으면 영원히 획득 불가능해진다.
function withLock(paths, name, fn) {
  ...
  const acquire = (stolen = false) => {
    try {
      ...
      if (fs.existsSync(markerPath)) {
        ...
        // 고아 락 폴더 — marker가 없으니 폴더 mtime이 유일한 단서다.
        try {
  ...
  // 반납 이중 보험: 콜백 안의 fail()이 process.exit를 치면 finally가 안 돈다
  // (실측: 만료 클레임 renew 거부 직후 락이 누출). exit 핸들러는 동기 코드를
  // 돌리므로 rmSync로 반납을 보장한다.
  const release = () => {
    try { fs.rmSync(lockPath, { recursive: true, force: true }); } catch { /* 이미 없음 */ }
  };
