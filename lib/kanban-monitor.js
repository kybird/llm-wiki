...
// 보드 신원(repo.docRoot)까지 돌려준다. 루프가 매 시작마다 monitor를 띄우므로
// 재기동은 멱등이어야 한다(멱등 재시도≠실패 — agent-cli-contract). 단 **같은
// 보드만 재사용한다** — 다른 프로젝트의 모니터를 가로채던 결함(2026-09-17
    ...
    const req = http.get({ host: '127.0.0.1', port, path: '/api/board', timeout: 1500 }, res => {
      ...
      res.on('end', () => {
        try {
          ...
          if (v.kind !== 'kanban-board') return resolve({ state: 'foreign' });
          resolve({ state: 'monitor', docRoot: v.repo && v.repo.docRoot, name: v.repo && v.repo.name });
        } catch { resolve({ state: 'foreign' }); }
      });
    });
    req.on('error', () => resolve({ state: 'none' }));
    req.on('timeout', () => { req.destroy(); resolve({ state: 'none' }); });
  });
}
...
function startMonitorServer(paths, port) {
