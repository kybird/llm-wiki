...
function renderBoardHtml(view) {
  ...
  const cardDiv = c => `<div class="card"><div class="t">${escapeHtml(c.title)}</div>${badges(c)}</div>`;
...
function boardReport(options = {}) {
  ...
  const report = {
    ...
    waitingQueue: review.map(c => c.meta.title),
  ...
  report.waitingQueue.forEach(t => console.log(`  • ${t}`));
