...
//   board                                     유도 뷰 (컬럼/WIP/대기/만료 클레임) — 플래그 없음
...
const CARD_NEW_FLAGS = { goal: 'string', ac: 'string', depends: 'string', 'not-before': 'string', kind: 'string', milestone: 'string' };
...
const USAGE = {
  ...
  'card edit': '사용법: llm-wiki card edit <제목> [--goal …] [--plan …] [--ac …] [--add-ac …] [--check-ac 1,2] [--note …] [--renew-claim] [--depends a,b] [--add-depends a,b] [--remove-depends a,b] [--milestone "<마일스톤 제목>" [--scope-amend "<범위 변경 사유>"]] [--remove-milestone]',
  ...
  unpick: '사용법: llm-wiki unpick <제목> --why "…" — 반납(doing→todo). 사유가 기록되지 않으면 집기 취소 패턴이 아침에 보이지 않는다.',
...
const UNPICK_FLAGS = { why: 'string' };

// ── milestone 검증 (card new/edit 공용) ───────────────────────────────────
// 소속 대상은 존재하고 kind: milestone이어야 하며, 마일스톤 카드 자신은 다른
// 마일스톤에 속할 수 없다(평면 — plan.md 3.8). 검증은 쓰기 전에 — 의존성과 같은
// 규칙(끊어진 소속은 회고 뷰를 영구히 비뚜러뜨린다).
function validateMilestoneTarget(paths, milestoneTitle, selfKind) {
  const target = kanban.findCard(paths, milestoneTitle);
  if (!target) {
    fail(`마일스톤 카드가 없다: ${milestoneTitle} — \`card new "${milestoneTitle}" --kind milestone\` 로 먼저 만들어라.`);
  }
  if (target.meta.kind !== 'milestone') {
    fail(`마일스톤 대상이 마일스톤 카드가 아니다: ${milestoneTitle} — 일반 카드에는 소속을 걸 수 없다.`);
  }
...
function cardNew(paths, config, { positional, flags }) {
  return kanban.withLock(paths, kanban.BOARD_LOCK_NAME, () => {
    ...
    // kind — 현재 유일한 값은 'milestone'. 일반 카드는 kind를 아예 안 적는다:
    // 기본값 필드는 전 카드에 누적되는 잡음이다.
    if (flags.kind !== undefined && flags.kind !== 'milestone') {
      fail(`--kind 값이 잘못됐다: ${flags.kind} — 쓸 수 있는 값: milestone`);
    }
    // 소속 검증도 쓰기 전에(같은 규칙) — 대상·평면 위반은 여기서 막힌다.
    const milestoneTitle = flagString(flags.milestone);
    if (milestoneTitle) validateMilestoneTarget(paths, milestoneTitle, flags.kind);
    ...
    if (flags.kind === 'milestone') meta.kind = 'milestone';
    ...
    // 계획 단위 소속 (plan.md 3.8) — 마일스톤의 진행 상태는 전부 유도된다.
    if (milestoneTitle) meta.milestone = milestoneTitle;
    ...
    console.log(`  ordinal: ${ordinal}${flags.kind === 'milestone' ? ', kind: milestone' : ''}${dependsOn.length ? `, depends_on: ${dependsOn.join(', ')}` : ''}${milestoneTitle ? `, milestone: ${milestoneTitle}` : ''}`);
  });
}
...
function cardEdit(paths, { positional, flags }) {
  return kanban.withLock(paths, kanban.BOARD_LOCK_NAME, () => {
    ...
    // --milestone 재지정 — 소속은 활성 카드에만 건다. 종결 카드의 소속은 회고
    // 뷰(마일스톤 패널·완료율)가 읽는 이력이라 소급 재편하지 않는다(plan.md 3.8).
    const milestoneTitle = flagString(flags.milestone);
    const scopeAmendReason = flagString(flags['scope-amend']);
    if (scopeAmendReason !== undefined && !milestoneTitle) {
      fail(`--scope-amend 는 --milestone 지정과 함께 쓴다 — 사유만으로는 소속이 정해지지 않는다.`);
    }
    if (milestoneTitle) {
      if (!kanban.ACTIVE_STATUSES.includes(cardObj.meta.status)) {
        fail(`종결 카드(${cardObj.meta.status})의 마일스톤은 바꾸지 않는다 — 회고 뷰가 읽는 이력이다: ${cardObj.filePath}`);
      }
      validateMilestoneTarget(paths, milestoneTitle, cardObj.meta.kind);
      if (meta.milestone !== milestoneTitle) {
        // ── 범위 봉인(scope seal, 2026-09-21 규약) ── 마일스톤 멤버는 계획 시점
        // (kanban-plan 분해)의 카드가 전부다 — 그 시점에 결승선이 확정된다. 멤버가
        // 하나라도 있는 마일스톤에 카드를 더 붙이는 건 범위 변경이라 명시적이어야
        // 한다. 활성 판정은 '멤버 중 하나라도 todo/doing/review'가 정의(규약 원문)
        // 이고, 전 멤버 종결 + 마일스톤 review 대기(자동 종결이 판정 뒤로 미뤄진
        // 상태)도 결승선은 확정됐으므로 같은 봉인 아래 둔다. 멤버 0장은 아직 계획
        // 분해 중 — 자유 소속. 자동 완료 동작은 바꾸지 않는다.
        const members = kanban.listAllCards(paths).filter(c => c.meta.milestone === milestoneTitle);
        const activeMembers = members.filter(c => kanban.ACTIVE_STATUSES.includes(c.meta.status));
        if (members.length && !scopeAmendReason) {
          fail(`진행 중 마일스톤에 카드 추가는 범위 변경이다: '${milestoneTitle}' — 멤버 ${members.length}장(활성 ${activeMembers.length}장). 결승선은 계획 시점(kanban-plan 분해)에 확정된다. 새 일이면 무소속 todo(card new 를 --milestone 없이)로 남기고 아침 의식에서 정리하라. 정말 범위를 넓히는 거면 \`--scope-amend "<사유>"\` 를 함께 줘라 — 사유는 카드 Notes에 '범위 변경'으로 남는다.`);
        }
        if (!members.length && scopeAmendReason) {
          fail(`--scope-amend 는 멤버가 있는(봉인된) 마일스톤에 붙일 때만 쓴다: '${milestoneTitle}' 는 아직 멤버가 없다 — 계획 시점이니 그냥 --milestone 로 붙여라.`);
        }
        if (members.length) {
          const notes = parsed.sections.get('Notes') || '';
          const line = `- ${nowIso()} — 범위 변경: 마일스톤 '${milestoneTitle}' 소속 추가 — ${scopeAmendReason}`;
          parsed.sections.set('Notes', notes ? `${notes}\n${line}` : line);
          changes.push('범위 변경(Notes 기록)');
        }
        meta.milestone = milestoneTitle;
...
function reopen(options = {}) {
  ...
  return kanban.withLock(paths, kanban.BOARD_LOCK_NAME, () => {
    ...
    // 마일스톤 역동기화 — 멤버가 되돌아왔는데 마일스톤이 이미 종결돼 있으면 같이
    // 되돌린다. 자동 종결의 역방향: 안 하면 종결된 계획 아래 살아있는 멤버라는
    // 썩은 상태(3.7 원칙 1)가 된다.
    if (meta.milestone) {
      const ms = kanban.findCard(paths, meta.milestone);
      if (ms && ms.meta.kind === 'milestone' && kanban.TERMINAL_FOLDERS.includes(ms.meta.status)) {
        const msMeta = kanban.parseFrontmatter(fs.readFileSync(ms.filePath, 'utf8'));
        const msParsed = kanban.parseBody(fs.readFileSync(ms.filePath, 'utf8'));
        msMeta.status = 'todo';
        delete msMeta.claimed_by;
        delete msMeta.claimed_at;
        const msNotes = msParsed.sections.get('Notes') || '';
        const msLine = `- ${nowIso()} — REVERTED: 멤버 '${meta.title}'의 reopen으로 마일스톤도 복귀`;
        msParsed.sections.set('Notes', msNotes ? `${msNotes}\n${msLine}` : msLine);
        fs.writeFileSync(path.join(paths.cardsDir, ms.fileName), kanban.serializeCard(msMeta, msParsed));
        fs.unlinkSync(ms.filePath);
        kanban.appendActivity(paths, { action: 'milestone-reverted', title: ms.meta.title, detail: `멤버 ${meta.title} reopen` });
        console.log(`Milestone reverted to todo: ${path.join(paths.cardsDir, ms.fileName)}`);
      }
    }
  });
}
...
// 지정 집기(--card)의 게이트 판정 — 통상 pick의 후보 판정과 같은 규칙(review →
// not_before → 클레임 → todo 의존)을 지정 카드 한 장에 적용한다. --card는 우선권이
...
function pickGateBlock(cardObj, resolvedTitles, config) {
  ...
  if (status === 'review') {
    return '사람 판정 대기(review)다 — resume으로 todo에 복귀시킨 뒤 집어라';
  }
  if (cardObj.meta.kind === 'milestone') {
    return '마일스톤 카드다 — 작업 항목이 아니라 목적 축이다. 소속 카드를 집어라 (plan.md 3.8)';
  }
...
function pick(options = {}) {
  ...
  const result = kanban.withLock(paths, kanban.BOARD_LOCK_NAME, () => {
    ...
    if (wantTitle) {
      ...
      for (const cardObj of active) {
        const status = cardObj.meta.status;
        // 마일스톤 카드는 작업 대상이 아니다(3.8) — 집을 게 없을 때 이유로는
        // 보고한다(사람이 보는 No pickable 상세).
        if (cardObj.meta.kind === 'milestone') {
          blocked.push({ title: cardObj.meta.title, isMilestone: true });
          continue;
        }
  ...
  if (!result.picked) {
    ...
    for (const b of result.detail) {
      ...
      else if (b.isMilestone) console.log(`  - ${b.title} — 마일스톤 카드(집기 대상 아님, plan.md 3.8)`);
    }
...
// ── 마일스톤 자동 종결 (done·supersede·abandon이 멤버 종결 뒤에 부른다) ────
// 마지막 멤버가 종결되는 순간 마일스톤도 종결한다 — 상태를 저장하면 반드시
// 어느 밤엔 잊히고 썩는다(3.7 원칙 1), 종결 사건이 일어나는 쓰기 안에서
// 동기화한다. review 대기 중이면 판정을 덮지 않고 보고만. 반드시 락 안에서.
function completeMilestoneIfAllTerminal(paths, milestoneTitle) {
  const ms = kanban.findCard(paths, milestoneTitle);
  if (!ms || ms.meta.kind !== 'milestone') {
    console.log(`  ⚠ 마일스톤 카드를 못 찾았다 — 소속 참조가 끊어졌다: ${milestoneTitle}`);
    return;
  }
  if (kanban.TERMINAL_FOLDERS.includes(ms.meta.status)) return; // 이미 종결된 이력
  const members = kanban.listAllCards(paths).filter(c => c.meta.milestone === milestoneTitle);
  if (members.some(c => kanban.ACTIVE_STATUSES.includes(c.meta.status))) return; // 진행 중 멤버 남음
  if (ms.meta.status === 'review') {
    console.log(`  마일스톤 전원 종결 — 다만 판정 대기(review) 중이라 자동 종결하지 않는다: ${milestoneTitle} — resume 뒤 다음 종결 사건에 종결된다`);
    return;
  }
  const counts = { done: 0, superseded: 0, abandoned: 0 };
  for (const m of members) counts[m.meta.status] += 1;
  const meta = { ...ms.meta };
  const parsed = { ...ms.card, ac: ms.card.ac.map(a => ({ ...a })), sections: new Map(ms.card.sections) };
  const result = `- ${nowIso()} — 전원 종결 자동 반영: 멤버 ${members.length}장 (done ${counts.done} · superseded ${counts.superseded} · abandoned ${counts.abandoned})`;
  const existing = parsed.sections.get('Result') || '';
  parsed.sections.set('Result', existing ? `${existing}\n${result}` : result);
  delete meta.claimed_by;
  delete meta.claimed_at;
  kanban.writeCard(ms, meta, parsed);
  kanban.moveCardTo(ms, paths.doneDir, 'done');
  kanban.appendActivity(paths, { action: 'milestone-done', title: ms.meta.title, detail: `멤버 ${members.length}장 전원 종결` });
  console.log(`  ◉ milestone 완료: '${ms.meta.title}' — 전원 종결 (${members.length}/${members.length}: done ${counts.done} · superseded ${counts.superseded} · abandoned ${counts.abandoned})`);
}
...
function doneCard(options = {}) {
  ...
  return kanban.withLock(paths, kanban.BOARD_LOCK_NAME, () => {
    ...
    // 마일스톤 소속이 있으면 전원 종결 확인 — 자동 종결은 이 쓰기 안에서(3.8).
    if (meta.milestone) completeMilestoneIfAllTerminal(paths, meta.milestone);
  });
}
...
function supersede(options = {}) {
  ...
  return kanban.withLock(paths, kanban.BOARD_LOCK_NAME, () => {
    ...
    if (parent.meta.milestone) completeMilestoneIfAllTerminal(paths, parent.meta.milestone);
  });
}
...
function abandon(options = {}) {
  ...
  return kanban.withLock(paths, kanban.BOARD_LOCK_NAME, () => {
    const cardObj = requireActiveCard(paths, title);
    // 마일스톤 폐기 가드 — 살아있는 멤버가 남은 계획을 조용히 버리지 않는다.
    // 회고 이력(3.3)을 지키려면 멤버를 먼저 종결하거나, 의도된 계획 접기면
    // 멤버부터 폐기하라.
    if (cardObj.meta.kind === 'milestone') {
      const activeMembers = kanban.listAllCards(paths)
        .filter(c => c.meta.milestone === cardObj.meta.title && kanban.ACTIVE_STATUSES.includes(c.meta.status));
      if (activeMembers.length) {
        fail(`마일스톤에 활성 멤버가 ${activeMembers.length}장 남아 있다: ${activeMembers.map(c => c.meta.title).join(', ')} — 멤버를 먼저 종결하거나 폐기하라.`);
      }
    }
    ...
    // 멤버 폐기도 종결 사건이다 — 마일스톤 전원 종결 확인(3.8).
    if (meta.milestone) completeMilestoneIfAllTerminal(paths, meta.milestone);
  });
}
...
function collectBoardAt(docRoot) {
  ...
  const byStatus = status => active
    .filter(c => c.meta.status === status && c.meta.kind !== 'milestone')
    .sort((a, b) => (Number(a.meta.ordinal) || 0) - (Number(b.meta.ordinal) || 0));

  // 소속 멤버는 4폴더 전역에서 유도한다 — 종결 멤버가 회고 뷰를 채운다(3.8).
  const membersByMilestone = new Map();
  const addMember = c => {
    const m = c.meta.milestone;
    if (!m) return;
    if (!membersByMilestone.has(m)) membersByMilestone.set(m, []);
    membersByMilestone.get(m).push({ title: c.meta.title, status: c.meta.status, claimedBy: c.meta.claimed_by || null });
  };
  active.forEach(addMember);

  // 종결 3폴더를 한 번만 훑는다 — 개수·최근 목록(모니터 종결 컬럼)·의존 해소
  // 판정·마일스톤 수집을 같은 패스에서 뽑는다(모니터가 2초 폴링으로 이 뷰를
  // 계속 요청한다). 최근 항목의 시각은 파일 mtime — 카드가 종결 폴더로 옮겨진
  // 시각이다.
  const terminalView = {
    counts: { done: 0, superseded: 0, abandoned: 0 },
    recent: [],
    resolvedTitles: [],
    milestones: [],
  };
  for (const [kind, dir] of [['done', paths.doneDir], ['superseded', paths.supersededDir], ['abandoned', paths.abandonedDir]]) {
    for (const c of kanban.listCardsInDir(dir)) {
      terminalView.counts[kind] += 1;
      if (kind !== 'abandoned') terminalView.resolvedTitles.push(c.meta.title);
      terminalView.recent.push({ title: c.meta.title, kind, at: fs.statSync(c.filePath).mtime.toISOString(), milestone: c.meta.milestone || null });
      addMember(c);
      if (c.meta.kind === 'milestone') terminalView.milestones.push({ card: c, at: fs.statSync(c.filePath).mtime.toISOString() });
    }
  }

  // 마일스톤 진행 상태는 전부 유도 — 저장하지 않는다(3.7 원칙 1·2). 활성
  // 마일스톤(ordinal순) 뒤에 종결 마일스톤(최근순).
  const msCounts = members => {
    const counts = { active: 0, done: 0, superseded: 0, abandoned: 0 };
    for (const m of members) {
      counts[kanban.ACTIVE_STATUSES.includes(m.status) ? 'active' : m.status] += 1;
    }
    return counts;
  };
  const msView = (c, completedAt) => {
    const members = (membersByMilestone.get(c.meta.title) || []);
    return {
      title: c.meta.title,
      status: c.meta.status,
      goal: c.card.goal || '',
      ordinal: Number(c.meta.ordinal) || 0,
      members,
      counts: msCounts(members),
  ...
  const milestones = [
    ...active
      .filter(c => c.meta.kind === 'milestone')
      .sort((a, b) => (Number(a.meta.ordinal) || 0) - (Number(b.meta.ordinal) || 0))
      .map(c => msView(c, null)),
    ...terminalView.milestones
      .sort((a, b) => (a.at < b.at ? 1 : -1))
      .map(({ card, at }) => msView(card, at)),
  ];
  ...
  const view = {
    ...
    // 목적 축 — 시간 축(terminal·terminalRecent)과 분리(3.8).
    milestones,
    terminal: terminalView.counts,
    // 의존 충족 판정용 종결 제목들 — done/superseded는 이미 해소(pick 판정과 일치).
    resolved: terminalView.resolvedTitles,
    // 모니터 종결 컬럼 — 완료가 쌓이는 모습이 수렴의 증거다(3.2). mtime 내림차순 8건.
    terminalRecent: terminalView.recent.sort((a, b) => (a.at < b.at ? 1 : -1)).slice(0, 8),
  };
  return { view, paths, config };
}

// board/video는 플래그를 받지 않는다 — 유도 뷰·타임랩스는 인자 없는 명령이다.
// bin이 --json을 전역 선추출해 dispatch로 넘기므로 여기서 명시적으로 실패하지
// 않으면 텍스트 보드를 뱉으며 exit 0 — 조용한 no-op 성공이다(2026-09-16 폐지:
// --html/--json 유도물 경로 제거와 함께 막았다).
const BOARD_FLAGS = {};
const VIDEO_FLAGS = {};
...
function boardView(options = {}) {
  ...
  validateFlags(flags, BOARD_FLAGS);
  const { view, config } = collectBoard();
...
function viewCard(cardObj, config) {
  ...
  return {
    ...
    // 모니터 뷰어가 경과 시간을 계산한다 — board --json 폐지(2026-09-16)로 이
    // 스키마를 소비하는 건 모니터뿐이라 노출이 안전해졌다.
    claimedAt: cardObj.meta.claimed_at || null,
    ...
    milestone: cardObj.meta.milestone || null,
  };
}
...
function renderBoard(view, config) {
  ...
  // 목적 축(3.8) — 활성 마일스톤은 멤버 상태 요약, 종결은 ✓ 한 줄. 컬럼에는
  // 마일스톤 카드가 안 나온다(collectBoard에서 제외).
  if (view.milestones.length) {
    console.log(`마일스톤 (${view.milestones.length})`);
    for (const m of view.milestones) {
      if (kanban.ACTIVE_STATUSES.includes(m.status)) {
        const finished = m.total - m.counts.active;
        const byMs = s => m.members.filter(x => x.status === s).length;
        console.log(`  • ${m.title} — ${finished}/${m.total} (todo ${byMs('todo')} · doing ${byMs('doing')} · review ${byMs('review')})`);
      } else {
        console.log(`  ✓ ${m.title} — ${m.total}/${m.total} (${m.status})`);
      }
    }
    console.log(line);
  }
...
// ── unpick — doing→todo 반납 (pick의 역수, 2026-09-22) ────────────────────
// 집었는데 내려놓는 원시형. 이 전에는 우회로가 전부 상태를 거짓으로 만들었다:
// handoff는 판정 목록(4종)용 파킹이라 review 큐와 노화 지표를 오염시키고
// (2026-09-12 실보드 사고 두 번의 수습책이었다 — doc/wiki/antipatterns/
// probing-side-effect-commands.md), 클레임 만료 대기는 doing 칸을 WIP 상한만큼
// 묶는다. 사유는 필수: 같은 벽(집었다 되돌리는 패턴)을 아침에 보게 하는 기록이다.
function unpick(options = {}) {
  const { paths } = requireBoard();
  const { positional, flags } = parseArgs(options.rest || []);
  const title = positional[0];
  const why = flagString(flags.why);
  if (!title || !why) fail(USAGE.unpick);
  validateFlags(flags, UNPICK_FLAGS);

  return kanban.withLock(paths, kanban.BOARD_LOCK_NAME, () => {
    const cardObj = kanban.findCard(paths, title);
    if (!cardObj) fail(`카드를 못 찾았다: ${title}`);
    if (cardObj.meta.status !== 'doing') {
      if (cardObj.meta.status === 'review') {
        fail(`review 카드는 unpick 대상이 아니다 — resume <제목> --note "판정 결과" 로 todo에 복귀시킨다: ${cardObj.filePath}`);
      }
      if (cardObj.meta.status === 'done') {
        fail(`done 카드는 unpick 대상이 아니다 — reopen <제목> --why "…" 로 되돌린다: ${cardObj.filePath}`);
      }
      if (cardObj.meta.status === 'todo') {
        fail(`이미 todo다 — 반납할 클레임이 없다: ${cardObj.filePath}`);
      }
      fail(`종결 카드(${cardObj.meta.status})는 반납 대상이 아니다: ${cardObj.filePath}`);
    }

    const meta = { ...cardObj.meta };
    const parsed = { ...cardObj.card, ac: cardObj.card.ac.map(a => ({ ...a })), sections: new Map(cardObj.card.sections) };
    meta.status = 'todo';
    delete meta.claimed_by; // 클레임 반납 — pick의 역수
    delete meta.claimed_at;
    const line = `- ${nowIso()} — UNPICKED: ${why}`;
    const notes = parsed.sections.get('Notes') || '';
    parsed.sections.set('Notes', notes ? `${notes}\n${line}` : line);

    kanban.writeCard(cardObj, meta, parsed);
    kanban.appendActivity(paths, { action: 'unpicked', title: meta.title, detail: String(why) });
    console.log(`Unpicked to todo: ${cardObj.filePath}`);
    console.log(`  사유: ${why}`);
  });
}
...
function boardReport(options = {}) {
  ...
  // review 큐 노화(2026-09-21 규약) — Handoff의 마지막 QUESTION 시각이 현재 대기의
  // 시작이다(handoff가 review로 파는 유일한 명령이고, 재파킹마다 QUESTION 가 더해진다).
  // activity.jsonl 은 10k줄 캡이 있어 오래된 이벤트가 잘리므로 정본인 카드 파일에서
  // 유도한다. 시각을 못 읽는 손편집 카드는 노화 미상(null)로 센다.
  const reviewWaitingDays = c => {
    const handoff = c.card.sections.get('Handoff') || '';
    const qLine = [...handoff.split(/\r?\n/)].reverse().find(l => l.includes('QUESTION:'));
    const m = qLine ? qLine.match(/^-\s+(.+?)\s+—\s+QUESTION:/) : null;
    const ts = m ? Date.parse(m[1]) : NaN;
    return Number.isNaN(ts) ? null : Math.floor((Date.now() - ts) / 86400000);
  };
  const waitingAges = review.map(reviewWaitingDays).filter(d => d !== null);
  const reviewAging = {
    oldestWaitingDays: waitingAges.length ? Math.max(...waitingAges) : null,
    over7Days: waitingAges.filter(d => d > 7).length,
    unknownAge: review.length - waitingAges.length,
  };

  // 마일스톤 경과일(생성일 기준) — 목적 축의 드래그 감지. 날짜(YYYY-MM-DD)끼리의
  // 로컬 자정差로 센다(offset 없는 date-time 은 로컬시간으로 파싱된다).
  const todayStart = Date.parse(`${localToday()}T00:00:00`);
  const milestoneAges = active
    .filter(c => c.meta.kind === 'milestone')
    .map(c => {
      const created = String(c.meta.created || '');
      const start = /^\d{4}-\d{2}-\d{2}$/.test(created) ? Date.parse(`${created}T00:00:00`) : NaN;
      return {
        title: c.meta.title,
        status: c.meta.status,
        elapsedDays: Number.isNaN(start) ? null : Math.floor((todayStart - start) / 86400000),
      };
    });
  ...
  const report = {
    ...
    // review 노화 지표(2026-09-21 규약) — 대기 큐가 마일스톤 완료를 막는 마지막
    // 요소다. 아침 의식의 첫 항목(review 삼진)이 보는 한 줄.
    reviewAging,
    milestoneAges,
    waitingQueue: review.map(c => {
      const v = viewCard(c, config);
      return { title: v.title, question: v.question, waitingDays: reviewWaitingDays(c) };
    }),
  ...
  const line = '────────────────────────────────────────';
  // 헤더 날짜는 localToday 로 유도 — generatedAt(UTC ISO)을 잘라 쓰면 시간대가
  // 하루 앞서며, 추이·경과일 같은 로컬 날짜 계산과 모순된다(2026-09-22 접수).
  // UTC 절대 시각은 기계용 JSON 필드(generatedAt)에만 남는다.
  console.log(`📊 Board report (${localToday()})`);
  console.log(line);
  ...
  const msAge = m => `'${m.title}' ${m.elapsedDays === null ? '?' : `${m.elapsedDays}일`}`;
  console.log(`마일스톤 경과(생성일 기준): ${report.milestoneAges.length ? report.milestoneAges.map(msAge).join(' · ') : '(없음)'}`);
  ...
  // 노화 지표(2026-09-21 규약) — review 삼진은 아침 의식의 첫 항목이다.
  const oldest = report.reviewAging.oldestWaitingDays;
  console.log(`REVIEW 노화 — 가장 오래된 대기 ${oldest === null ? '없음' : `${oldest}일`} · 7일 초과 ${report.reviewAging.over7Days}건${report.reviewAging.over7Days ? '  ⚠ 아침 의식 첫 항목: review 삼진 — 사람이 답한다/버린다' : ''}`);
  if (!report.waitingQueue.length) console.log('  (비어 있음 — 밤샘 동안 판단 질문이 없었다는 뜻)');
  for (const w of report.waitingQueue) {
    const age = w.waitingDays === null ? '' : ` (대기 ${w.waitingDays}일${w.waitingDays > 7 ? ' ⚠' : ''})`;
    console.log(`  • ${w.title}${age}`);
...
function boardVideo(options = {}) {
  const { paths, config } = requireBoard();
  // 미검증 플래그를 조용히 무시하던 결함(2026-09-16 설계 리뷰에서 접수) —
  // video는 플래그를 받지 않는다. bin이 전역 선추출한 --json도 여기로 넘어온다.
  const flags = { ...parseArgs(options.rest || []).flags };
  if (options.json) flags.json = true;
  validateFlags(flags, VIDEO_FLAGS);
  const activity = kanban.readActivity(paths);
