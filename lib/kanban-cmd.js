...
function cardNew(paths, config, { positional, flags }) {
  return kanban.withLock(paths, kanban.BOARD_LOCK_NAME, () => {
    ...
    const dependsOn = stringArray(flags.depends).flatMap(s => s.split(',')).map(s => s.trim()).filter(Boolean);
...
function cardEdit(paths, { positional, flags }) {
  return kanban.withLock(paths, kanban.BOARD_LOCK_NAME, () => {
    ...
    if (typeof flags.goal === 'string') parsed.goal = flags.goal;
    if (typeof flags.plan === 'string') parsed.sections.set('Plan', flags.plan);
