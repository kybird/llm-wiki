...
function collectBoard() {
  ...
  // 판정을 같은 패스에서 뽑는다(모니터가 2초 폴링으로 이 뷰를 계속 요청한다).
  // 최근 항목의 시각은 파일 mtime — 카드가 종결 폴더로 옮겨진 시각이다.
  const terminalView = { counts: { done: 0, superseded: 0, abandoned: 0 }, recent: [], resolvedTitles: [] };
  for (const [kind, dir] of [['done', paths.doneDir], ['superseded', paths.supersededDir], ['abandoned', paths.abandonedDir]]) {
    for (const c of kanban.listCardsInDir(dir)) {
      ...
      terminalView.recent.push({ title: c.meta.title, kind, at: fs.statSync(c.filePath).mtime.toISOString() });
    }
  }
