...
function listNewLogs() {
  ...
  if (fs.existsSync(indexPath)) {
    ...
    if (match) lastUpdated = new Date(match[1]);
  }

  console.log(`Last compiled date: ${lastUpdated.toISOString().split('T')[0]}`);
  ...
  const files = fs.readdirSync(rawRoot)
    ...
    .map(f => {
      const stats = fs.statSync(path.join(rawRoot, f));
      return { name: f, mtime: stats.mtime };
    })
    .filter(f => f.mtime > lastUpdated)
    .sort((a, b) => a.mtime - b.mtime);

  if (files.length > 0) {
    console.log('\nNew/Modified logs to process:');
    files.forEach(f => console.log(`- ${f.name} (${f.mtime.toISOString().split('T')[0]})`));
  } else {
