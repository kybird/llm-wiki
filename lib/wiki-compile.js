...
// "새 로그" 데이터 수집 — 판정 기준은 파일 mtime이 아니라 로그 헤더 날짜(# YYYY-MM-DD) —
// git checkout으로 mtime이 머신마다 갈리는 중복/누락과, 같은 날 컴파일해도 남는
// same-day 오탐을 함께 없앤다. 헤더 날짜가 없는 파일은 mtime 날짜로 폴백.
// ISO 날짜 문자열 비교는 달력 순서와 일치.
function collectNewLogs(docRoot) {
  ...
  if (fs.existsSync(rawRoot)) {
    fs.readdirSync(rawRoot)
      ...
      .forEach(f => newLogs.push(f));
  }

  return { schemaVersion: 1, kind: 'compile-list', lastCompiled: lastUpdated, newLogs };
}
...
function listNewLogs() {
  ...
  data.newLogs.forEach(f => console.log(`- ${f.name} (${f.date})`));
  return data;
}
