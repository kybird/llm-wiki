...
//   - 2026-09-22: execFileSync import 누락 수습(214e8c2 도입 당시부터 시맨틱층 전면
//     사망 — ReferenceError가 catch에 삼켜져 "No semantic results."로 위장). 아울러
//     catch가 에러를 먹지 않고 봉투의 semantic.errors에 실어 렌더에 드러낸다 —
//     빈 결과(정상 한계)와 고장을 못 가리면 죽은 층이 성공으로 위장한다.
...
function collectQmdResults(query, config) {
  ...
  if (!qmdPath) return { available: false, collections: [], errors: [] };
  ...
  for (const coll of collections) {
    try {
      ...
      // 삼키면 안 된다 — 빈 결과(정상 한계)와 고장이 같은 출력이 되면 죽은 시맨틱층이
      // "No semantic results."로 위장한다(2026-09-22, 214e8c2 이후 12일의 실측).
      errors.push({
        collection: coll,
  ...
  return { available: true, collections: results, errors };
}
...
function renderSearchText(result) {
  ...
  if (semantic.collections.length === 0 && errors.length === 0) {
  ...
  // 고장은 결과 부재와 다른 줄로 나간다 — "No semantic results."가 정상 한계인지
  // 죽은 층인지 사람이 한눈에 구분하게.
  for (const e of errors) {
    console.log(`(QMD error on ${e.collection}: ${e.message.replace(/\s+/g, ' ')})`);
  }
}
